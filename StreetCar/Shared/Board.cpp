	...
	*sHand = Tile(Empty);
	...
	if( station != NULL){
		...
		if (!(station->isLinked())){
			...
			sBoard->setStop(true);
			if (station->getCoordinates().x - sBoard->getCoordinates().x == 1)
				station->setOrientation(WEST);
			else if (station->getCoordinates().x - sBoard->getCoordinates().x == -1)
				station->setOrientation(EAST);
			else if (station->getCoordinates().y - sBoard->getCoordinates().y == 1)
