...
Computer::Computer(std::vector<Player> allPlayers,int IAm){
    ...
    cout << endl;
    Stop stop1(StationL),stop2(StationA);
...
vector<Stop> Computer::createOrder(){
    ...
    bool firstIsLeft=(myTerminus[0][0].x<myTerminus[1][0].x),firstIsDown=(myTerminus[0][0].y<myTerminus[1][0].y),isVert=(myTerminus[0][0].y==myTerminus[0][1].y);
	...
	distanceTmp=ABS(INFORMATIONS.itinerary[i].coordinates.x - myTerminus[0][0].x)+ABS(INFORMATIONS.itinerary[i].coordinates.y - myTerminus[0][0].y);
	...
	for(int j=0;j<INFORMATIONS.itinerary.size();j++){
		...
		distanceTmp=ABS(INFORMATIONS.itinerary[j].coordinates.x - StationOrder[i].coordinates.x)+ABS(INFORMATIONS.itinerary[j].coordinates.y - StationOrder[i].coordinates.y);
		if(distanceTmp<distance){
		    distance=distanceTmp;
