...
void *serverOutputHandler(void* argv){
	...
	while (!isFinish){
		...
		cout << "C: type message " << readPack->toString() << endl;

		cout << "C: message -------------- " << ss.str() << endl;
		int g = htonl(size);
		n = write(sockfd, (const char*)&g, sizeof(int));
		n = write(sockfd, ss.str().c_str(), size);
		cout << "C: write on network " << endl;

		if (n < 0) 
			cout << "C: ERROR writing from socket" << endl;
		delete readPack;
	}
...
void *serverInputHandler(void* argv){
	...
	while (!isFinish){
		...
		cout << "C: Wait message" << endl;
		int a ;
		n = recv(newsockfd,(char*)&a,sizeof(int),MSG_WAITALL);
		a = ntohl(a);
		cout << "C: receive int a = " << a << endl;
		n = recv(newsockfd,buffer,a,MSG_WAITALL);
		if (n > 0) {

			cout << "C: reading on socket " << n << " " << buffer << endl;
			buffer[n] = '\0';
			...
			switch((packs)i){ 
				...
				default:
					pack->idPack = (packs)i;
					cout << "C: deserialisable error " << pack->toString() << endl;
					break;
			}
			cout << "C: this is pack : "<< pack->toString() << endl;
			prodConsInput->produce(pack);
		}
		else {
			cout << "C: ERROR reading from socket" << endl;
			isFinish = true;
			return 0;
		}
	}
